                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" id="post-list">
                        ...
                        <div class="relative bg-gray-900 block p-6 border border-gray-100 rounded-lg max-w-sm mx-auto mt-24" href="#">
                            ...
                            <div class="my-4">
                                ...
                                <h2 class="text-gray-300 text-2xl font-bold py-1">{{ $adoptionRequest->user->first_name }} {{ $adoptionRequest->user->name }} </h2>
                            ...
                            <div class="flex justify-end p-3">
                                <form action="{{ route('adoptionRequests.accept', $adoptionRequest->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to accept this request?');">
                                    @csrf
                                    <button type="submit" class="px-2 py-1 mr-5 text-white bg-green-500 border border-gray-200 font-semibold rounded hover:bg-green-700">Accept</button>
                                </form>
                                <form action="{{ route('adoptionRequests.refuse', $adoptionRequest->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to refuse this request?');">
                                    @csrf
                                    <button type="submit" class="px-2 py-1 text-white bg-red-500 border border-gray-200 font-semibold rounded hover:bg-red-700">Refuse</button>
                                </form>
                            </div>
                        </div>
                        @endforeach
